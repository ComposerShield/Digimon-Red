...
; after using a move, decrement pp in battle and (if not transformed?) in party
	ld a, [de]
	cp STRUGGLE
	ret z                ; if the DIGIMON is using "struggle", there's nothing to do
	                     ; we don't decrement PP for "struggle"
	ld hl, wPlayerBattleStatus1
	ld a, [hli]          ; load the wPlayerBattleStatus1 DIGIMON status flags and increment hl to load the
...
; decrement PP in the party struct
	...
	bit TRANSFORMED, a
	ret nz               ; Return if transformed. Digimon Red stores the "current DIGIMON's" PP
	                     ...
	                     ; modification to the "party's DIGIMON" PP that we made to the
	                     ; "current DIGIMON's" PP.  But, if we're dealing with a Transformed
