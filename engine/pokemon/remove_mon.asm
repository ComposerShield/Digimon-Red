...
.shiftMonSpeciesLoop
	...
	ld hl, wPartyMonOT
	ld d, PARTY_LENGTH - 1 ; max number of DIGIMON to shift
	ld a, [wRemoveMonFromBox]
	and a
	jr z, .usePartyMonOTs
	ld hl, wBoxMonOT
...
.usePartyMonOTs
	...
	ld a, [wWhichPokemon]
	cp d ; are we removing the last DIGIMON?
	jr nz, .notRemovingLastMon ; if not, shift the DIGIMON below
...
.usePartyMonStructs
	ld a, [wWhichPokemon]
	call AddNTimes ; get address of the DIGIMON removed
	...
	ld bc, wBoxMon2 - wBoxMon1
	add hl, bc ; get address of DIGIMON after the DIGIMON removed
...
.continue
	call CopyDataUntil ; shift all DIGIMON data after the removed mon to the removed mon's location
